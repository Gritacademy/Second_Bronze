...
static TAutoConsoleVariable<int32> CVarOculusIncreaseThreadPrio(
	...
	0,
	TEXT("This CVar only works when set using the project config\n")
...
namespace OculusXRHMD
{
	...
	void FOculusXRHMD::DrawHiddenAreaMesh(class FRHICommandList& RHICmdList, int32 ViewIndex) const
	{
		DrawHiddenAreaMesh(RHICmdList, ViewIndex, 1);
	}

	void FOculusXRHMD::DrawVisibleAreaMesh(class FRHICommandList& RHICmdList, int32 ViewIndex) const
	{
...
#if !UE_VERSION_OLDER_THAN(5, 5, 0)
	void FOculusXRHMD::GetMotionControllerState(UObject* WorldContext, const EXRSpaceType XRSpaceType, const EControllerHand Hand, const EXRControllerPoseType XRControllerPoseType, FXRMotionControllerState& MotionControllerState)
	{
		...
		MotionControllerState.DeviceName = IOculusXRHMDModule::NAME_OculusXRHMD;
			...
			{
				{
					...
					FVector Position = FVector::ZeroVector;
					FRotator Rotation = FRotator::ZeroRotator;
					const FTransform TrackingToWorld = XRSpaceType == EXRSpaceType::UnrealWorldSpace ? GetTrackingToWorldTransform() : FTransform::Identity;
					const float WorldToMeters = XRSpaceType == EXRSpaceType::UnrealWorldSpace ? GetWorldToMetersScale() : 100.0f;
					bSuccess = MotionController->GetControllerOrientationAndPosition(0, MotionSource, Rotation, Position, WorldToMeters);
					if (bSuccess)
					{
	...
	bool FOculusXRHMD::GetRecommendedVelocityTextureSize(FIntPoint& OutTextureSize)
	{
		CheckInRenderThread();
		...
		if (EyeLayer_RenderThread.IsValid())
		{
			...
			if (SwapChain.IsValid())
			{
				...
				if (Texture->IsValid())
				{
					OutTextureSize = Texture->GetDesc().Extent;
					return true;
				}
			}
		}
	...
	bool FOculusXRHMD::AllocateVelocityTexture(uint32 Index, const FIntPoint& Size, uint8 Format, uint32 NumMips, ETextureCreateFlags TexFlags, FTextureRHIRef& OutTexture, uint32 NumSamples)
	{
		CheckInRenderThread();
		check(Index == 0);
		...
		if (EyeLayer_RenderThread.IsValid())
		{
			...
			if (SwapChain.IsValid())
			{
				...
				// Only set texture and return true if we have a valid texture of compatible size
				if (Texture->IsValid() && Texture->GetDesc().Extent == Size && Texture->GetDesc().Format == Format)
				{
					if (bNeedReAllocateMotionVectorTexture_RenderThread)
					{
						UE_LOG(LogHMD, Log, TEXT("[Mobile SpaceWarp] Allocating Oculus %d x %d motion vector swapchain"), Size.X, Size.Y);
	...
	bool FOculusXRHMD::AllocateVelocityDepthTexture(uint32 Index, const FIntPoint& Size, uint8 Format, uint32 NumMips, ETextureCreateFlags TexFlags, FTextureRHIRef& OutTexture, uint32 NumSamples)
	{
		CheckInRenderThread();
		check(Index == 0);
		...
		if (EyeLayer_RenderThread.IsValid())
		{
			const FXRSwapChainPtr& SwapChain = EyeLayer_RenderThread->GetMotionVectorDepthSwapChain();
			if (SwapChain.IsValid())
			{
				FTextureRHIRef Texture = SwapChain->GetTexture2DArray() ? SwapChain->GetTexture2DArray() : SwapChain->GetTexture2D();
				...
				if (Texture->IsValid() && Texture->GetDesc().Extent == Size && Texture->GetDesc().Format == Format)
				{
					if (bNeedReAllocateMotionVectorDepthTexture_RenderThread)
					{
						UE_LOG(LogHMD, Log, TEXT("[Mobile SpaceWarp] Allocating Oculus %d x %d motion vector depth swapchain"), Size.X, Size.Y);
						bNeedReAllocateMotionVectorDepthTexture_RenderThread = false;
					}
...
#if defined(WITH_OCULUS_BRANCH)
		InViewFamily.bRenderStereoVelocity = SupportsSpaceWarp() ? CVarOculusEnableSpaceWarpUser.GetValueOnAnyThread() != 0 : false;
#endif // defined(WITH_OCULUS_BRANCH)
	}
	...
	bool FOculusXRHMD::InitializeSession()
	{
		...
		if (!FOculusXRHMDModule::GetPluginWrapper().GetInitialized())
		{
			...
			bNeedReAllocateMotionVectorTexture_RenderThread = false;
			bNeedReAllocateMotionVectorDepthTexture_RenderThread = true;
	...
	void FOculusXRHMD::UpdateStereoRenderingParams()
	{
		...
		else
		{
			...
			Settings->SetPixelDensity(PixelDensityCVar ? PixelDensityCVar->GetFloat() / 100 : 1.0f);
#endif
		...
		*EyeLayerFound = MakeShareable(EyeLayer);
		...
		int SpaceWarpAllocateFlag = SupportsSpaceWarp() ? ovrpLayerFlag_SpaceWarpDataAllocation | ovrpLayerFlag_SpaceWarpDedicatedDepth : 0;
				...
				&EyeLayerDesc)))
		{
			...
			bool bSpaceWarp = false;
			if (SupportsSpaceWarp())
			{
				bSpaceWarp = CVarOculusEnableSpaceWarpUser.GetValueOnAnyThread() != 0;
			}
			EyeLayer->SetEyeLayerDesc(EyeLayerDesc, bSpaceWarp);
	...
	void FOculusXRHMD::InitializeEyeLayer_RenderThread(FRHICommandListImmediate& RHICmdList)
	{
		...
		if (LayerMap[0].IsValid())
		{
			...
			if (EyeLayer->GetMotionVectorDepthSwapChain().IsValid())
			{
				if (!EyeLayer_RenderThread.IsValid() || EyeLayer->GetMotionVectorDepthSwapChain() != EyeLayer_RenderThread->GetMotionVectorDepthSwapChain())
				{
					bNeedReAllocateMotionVectorDepthTexture_RenderThread = true;
					UE_LOG(LogHMD, VeryVerbose, TEXT("[Mobile SpaceWarp]  request to re-allocate motionVector depth textures"));
				}
			}
	...
	void FOculusXRHMD::EnableHardOcclusions(bool bEnable)
	{
		if (bEnable)
		{
			OculusXRTelemetry::TScopedMarker<OculusXRTelemetry::Events::FEnableHardOcclusions>();
		}
	...
	void FOculusXRHMD::EnableSoftOcclusions(bool bEnable)
	{
		if (bEnable)
		{
			OculusXRTelemetry::TScopedMarker<OculusXRTelemetry::Events::FEnableSoftOcclusions>();
		}
	...
	{
		// NOTE: We should *NOT* use Frame_RenderThread->WorldToMetersScale here!
		// GNearClippingPlane is always expressed in cm regardless of the WorldToMetersScale setting when using `FOculusXRHMD`.
		// This is not true when using OpenXR Native, so the code in FEnvironmentDepthExtensionPlugin::ComputeEnvironmentDepthParameters_RenderThread
		// is different.
		float ScreenNearZ = GNearClippingPlane / 100.f;
...
#define BOOLEAN_COMMAND_HANDLER_BODY(ConsoleName, FieldExpr)                        \
	...
	void FOculusXRHMD::LoadFromSettings()
	{
		...
		Settings->bFaceTrackingVisemesEnabled = HMDSettings->bFaceTrackingVisemesEnabled;
	}
	void FOculusXRHMD::LogEnabledFeatures() const
	{
		...
		UE_LOG(LogHMD, Log, TEXT("EmulatedUniformBuffers: %s"), IsUsingEmulatedUniformBuffers(Settings->CurrentShaderPlatform) ? TEXT("Enabled") : TEXT("Disabled"));
#endif // WITH_OCULUS_BRANCH
